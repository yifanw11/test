...
control Ingress(
    ...
    apply {
        ...
        if (ig_md.traffic_type == USER) {
            ...
            if ((hdr.ethernet.ether_type == ETHERTYPE_ARP || hdr.vlan_dual_tag.ether_type_ctag == ETHERTYPE_ARP)) {
                port_forward.apply();
            }
            ...
            // port_forward()
            port_forward.apply();
            // // Handles ARP responses from RSP to user
            // if ((hdr.ethernet.ether_type == ETHERTYPE_ARP || hdr.vlan_dual_tag.ether_type_ctag == ETHERTYPE_ARP)) {
            //     arp_forward.apply();
            // }
            // // Handles normal downstream traffic from RSP, same subnet so MAC change not applicable
            // else{
            //     port_forward.apply();
            //     //ipv4_forward.apply();
            // }
